...
const MovieBookingForm = () => {
    ...
    return (
        <div className="flex flex-col md:flex-row mt-10 bg-indigo-300 rounded-lg justify-between">
            <div className="md:w-full">
                <div className="text-2xl text-white font-bold p-3 bg-indigo-700 rounded-t-lg md:rounded-none md:rounded-tl-lg">
                    You're booking:{" "}
                    <span className="text-4xl text-yellow-300"> {show.name}</span>
                </div>
                ...
                <div className="my-6 mx-3">
                    ...
                    {show.rating && (
                        <div className="font-semibold">
                            Average Rating:{" "}
                            <span className="font-bold"> {show.rating.average}</span>
                        </div>
                    )}
                    ...
                    {show.schedule && (
                        <div className="font-semibold">
                            Schedule Time:{" "}
                            <span className="font-bold"> {show.schedule.time}</span>
                        </div>
                    )}
                    ...
                    {show.schedule && (
                        <div className="font-semibold">
                            Schedule Days:{" "}
                            <span className="font-bold"> {show.schedule.days}</span>
                        </div>
                    )}
                </div>
                ...
                >
                    ...
                    <button
                        type="submit"
                        className="text-xl font-bold bg-yellow-300 hover:bg-yellow-200 duration-200 w-40 rounded ml-6 mt-6 p-2"
                    >
                        Book Movie
                    </button>
                </form>
